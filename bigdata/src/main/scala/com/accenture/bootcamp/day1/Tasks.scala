...
trait Tasks {
  ...
  /**
    ...
    */
  def task5(): Long = {
    Tokenizer.countWords(australianTreaties)
  ...
  def task6(): Long = {
    ...
    Tokenizer.countWords(australianTreaties) + Tokenizer.countWords(newYearHonours)
  }
  ...
  def task8(): Long = {
    ...
    Tokenizer.numbers(australianTreaties).distinct().map(_.toLong).count()

    // val yo = Tokenizer.numbers(australianTreaties).equals(uniqueval).map(x => (x)).foreach { println }

    // Tokenizer.numbers(australianTreaties)_
  }
  ...
  def task9(): Double = {
    ...
    Tokenizer.numbers(australianTreaties).mean()
  }
